...
df = pd.read_csv("/Users/Sarah/Desktop/GT/Spring 2021/CS 7641/Project/Data/cleaned_data_2/state_county_info_cleaned.csv")

df_filtered = df.dropna(axis=0) # drop any rows with nan or missing values
df_filtered = df_filtered.drop(['Unnamed: 0'], axis=1) # drop these columns- I think we don't need the data in them
...
y = df_filtered.loc[:,['state', 'county', 'cases', 'date']].values # y is target values/labels- the number of cases for each state

# Separate out the features
headers = list(df_filtered.columns)# get all the column names
headers.remove("state") # exclude these two columns because they are the targets
headers.remove("county")
headers.remove("cases")
headers.remove("date")
...
# pca = PCA(n_components = 10) # do PCA with all components
pca = PCA()
...
plt.title('County Demographics Scree Plot')
plt.xlabel("Principal Component")
plt.ylabel("Variance")

# Reconstruct data
reconstructDf = pd.DataFrame(data = reconstructX) # data reconstructed using only reduced features
finalDf = pd.concat([reconstructDf, df_filtered[['state', 'county', 'cases', 'date']]], axis = 1) # add the states and total cases values back in
finalDf.to_csv(r'/Users/Sarah/Desktop/GT/Spring 2021/CS 7641/Project/PCA/with_dates/states_county_info_34_components_with_dates.csv', index=False)